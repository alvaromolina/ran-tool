                ...
                ui.layout_sidebar(  
                    ui.sidebar(
                        ui.input_select("select_technology_voice", "Technology Filter", {
                            'ALL':'ALL', '3G':'3G', '4G':'4G'  # Removed '5G':'5G'
                        }, selected='ALL'),
...
def server(input, output, session):
    ...
    # Chart functions (one of each)
    @output
    @render_widget
    def chart01():  # CELLCHANGE - Fix date range
        site_att = selected_site_att.get()
        if site_att is None:
            site_att = 'DIFALO0001'

        # Fix: Use both min_date and max_date inputs
        min_date = input.min_date().strftime("%Y-%m-%d")
        max_date = input.max_date().strftime("%Y-%m-%d")
        dates = [min_date, max_date]  # Use actual min_date instead of hardcoded "2024-01-01"
    ...
    @output
    @render_widget
    def chart_cqi():  # CQI - Added technology filter
        ...
        # Convert technology filter
        technology_filter = None if technology_cqi == 'ALL' else technology_cqi
        
        # Get CQI data for the selected site with date and technology filtering
        ...
        # Update title to include technology filter
    ...
    @output
    @render_widget
    def chart_traffic_data():  # TRAFFICDATA - Added technology filter
        ...
        # Get traffic data for the selected site with date and technology filtering
        traffic_data = get_traffic_data_daily(
            att_name=site_att,
            min_date=min_date, 
            max_date=max_date, 
            technology=technology_filter,
            vendor=None  # Show all vendors
        )
    ...
    @output
    @render_widget
    def chart_traffic_voice():  # TRAFFICVOICE - Added technology filter
        ...
        # Get voice traffic data for the selected site with date and technology filtering
