...
from lte_cqi_site_group_processor import get_lte_cqi_for_site_group
...
def calculate_period_average(site_att, start_date, end_date):
    """
    Calculate average LTE CQI for a single site in a given period.
    """
    engine = get_engine()
    
    try:
        query = """
        SELECT AVG(lte_cqi) as avg_lte_cqi, COUNT(*) as record_count
        ...
        if df.empty or df.iloc[0]['avg_lte_cqi'] is None:
            return None, 0
        
        return float(df.iloc[0]['avg_lte_cqi']), int(df.iloc[0]['record_count'])
...
def calculate_neighbors_group_average(neighbors, start_date, end_date):
    """
    Calculate average LTE CQI for neighbors using the group processor.
    """
    if not neighbors:
        return None, 0
    ...
    try:
        ...
        try:
            ...
            df_group = get_lte_cqi_for_site_group(
                site_list=neighbors,
        ...
        # Get the LTE CQI value from the group calculation
        avg_lte_cqi = df_group['lte_cqi'].iloc[0]
        record_count = df_group['site_count'].iloc[0]
        
        return float(avg_lte_cqi), record_count
...
def get_max_date_available():
    """
    Get the maximum date available in lte_cqi_metrics_daily table.
    """
    engine = get_engine()
    
    try:
        query = "SELECT MAX(date) as max_date FROM lte_cqi_metrics_daily"
...
def lte_cqi_site_evaluation(input_date, site_att, guard=7, period=7, thd_low=5, thd_high=5):
    """
    Evaluate LTE CQI patterns for a specific site before and after a given date.
    ...
    if max_date is None:
        return {"error": "No data available in lte_cqi_metrics_daily table"}
...
def lte_cqi_neighbor_evaluation(input_date, site_att, radius=5, guard=7, period=7, thd_low=5, thd_high=5):
    """
    Evaluate LTE CQI patterns for neighbors of a specific site before and after a given date.
    Uses the lte_cqi_group_processor for efficient neighbor calculations.
    ...
    # Get neighbors within radius using imported function - FIXED: pass single site, not list
...
def print_evaluation_summary(result):
    ...
    title = f"LTE CQI {eval_type.upper()} EVALUATION SUMMARY"
...
if __name__ == "__main__":
    ...
    site_result = lte_cqi_site_evaluation(
    ...
    neighbor_result = lte_cqi_neighbor_evaluation(
        input_date=test_date,
        site_att=test_site,
        radius=5,
        guard=7,
